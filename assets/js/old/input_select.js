...
InputSelectHooks.InputOrSelectOne = {
	...
	initInputOrSelectOne() {
		...
		let $select =
			this.el.querySelector("select.tagify") ||
			this.previousSiblingSelector(this.el, "select.tagify");
			...
			callbacks: {
				input: (e) => {
					if (phxAutocomplete && e.detail.value) {
						...
						this.pushEventTo("#" + id, phxAutocomplete, {
				...
				add: (e) => {
					if (e.detail.data) {
						this.pushEventTo("#" + id, "tagify_add", {
				...
				remove: (e) => {
					if (e.detail.data) {
						this.pushEventTo("#" + id, "tagify_remove", {
							id: e.detail.data.value,
						});
					}
				},
	...
	updated() {
		...
		if (select_outside) {
			// FIXME: doesn't work because we can't have phx-ignore and also update the input value in LV
			console.log("update input_select suggestions");
			const tagify = window.InputOrSelectOnes[this.el.id];
			// tagify.loadOriginalValues();

			tagify.whitelist = this.suggestionsFromSelect(
				select_outside,
				tagify.whitelist,
			); // add already-existing tags to the new whitelist array
