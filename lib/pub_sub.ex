defmodule Bonfire.PubSub do
  ...
  defp do_broadcast(topic, data) do
    ...
    Phoenix.PubSub.broadcast(Bonfire.PubSub, Utils.maybe_to_string(topic), data)
  end

  @doc "Broadcast while attaching telemetry info. The receiving module must `use Bonfire.PubSub` to correctly unwrap the Event"
  defmacro broadcast_with_telemetry(topic, message) do
    quote do
      current_function = Bonfire.PubSub.current_function(__ENV__)
      Bonfire.PubSub.broadcast_with_telemetry(unquote(topic), unquote(message), current_function)
    end
  end
  ...
  def broadcast_with_telemetry(topic, message, source) do
    ...
    OpenTelemetry.Tracer.with_span "bonfire.pubsub:broadcast", opts do
      ...
      Bonfire.PubSub.broadcast(topic, event)
    end
  end

  defmacro __using__(_opts) do
    quote do
      def handle_info(%Bonfire.PubSub.Event{} = event, socket) do
        require OpenTelemetry.Tracer
  ...
  defp socket_connected_or_user?(%Phoenix.LiveView.Socket{} = socket),
    do: UI.Common.socket_connected?(socket)
