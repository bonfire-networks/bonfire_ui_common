if Bonfire.Common.Extend.module_enabled?(LiveViewNative) do
  defmodule Bonfire.UI.Common.Web.Native.Original do
    @moduledoc """
    ...
    import Bonfire.UI.Common.Web, only: [verified_routes: 0]
    ...
    def live_view() do
      quote do
        use LiveViewNative.LiveView,
          ...
          layouts: [
            swiftui: {Bonfire.UI.Common.Web.Layouts.SwiftUI, :app}
          ]
    ...
    defp helpers(format) do
      gettext_quoted =
        quote do
          import Bonfire.UI.Common.Web.Gettext
        end
      ...
      core_component_module =
        Module.concat([Bonfire.UI.Common.Web, CoreComponents, plugin.module_suffix])
