...
let ColourPicker = {
	...
	mounted() {
		...
		this.onApply = (event) => {
			const root = event.currentTarget.closest("div[id^='color-']");
			const input = root && root.querySelector("hex-input");
			if (!root || !input) return;
			this.pushEvent("Bonfire.Common.Settings:put_custom_color", {
				keys: "ui:theme:custom:" + root.id,
				values: input.color,
				// data-scope is on the picker node, since it's teleported away from #custom-themes
