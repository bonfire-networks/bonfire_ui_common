...
@avatar_colors [
  ...
  def avatar(seed, opts \\ [size: 150, avatar_colors: @avatar_colors, background_colors: @background_colors, blackout: true, round: true]) do
    ...
    create_avatar = fn -> Enum.map(shapes, &Enum.random(&1))
      |> List.flatten()
      |> Enum.map(&(&1.(avatar_color)))
      |> Enum.join("")
    end
    create_svg(opts[:size], [create_background(opts[:round], background_color), create_avatar.(), (if opts[:blackout], do: create_blackout(opts[:round]), else: "")])
  end
...
defp seed_randomiser(seed) do
        :rand.seed(:exsplus, (seed)  |> to_charlist() |> Enum.take(3) |> List.to_tuple) # let's seed the random algorithm
