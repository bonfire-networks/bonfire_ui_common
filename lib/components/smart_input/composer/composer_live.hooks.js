...
export default {
	mounted() {
		...
		// Restore a locally-saved draft (saved/keyed/cleared by bonfire_live.js) —
		// mirrors the Milkdown editor's mount restore
		const draft = window.Bonfire?.getComposerDraft?.(textarea);
		if (draft) {
			textarea.value = draft;
			textarea.parentNode.dataset.value = draft; // keep input-sizer in sync
		}
