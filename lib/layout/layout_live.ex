defmodule Bonfire.UI.Common.LayoutLive do
  ...
  def render(assigns) do
    ...
    |> debug("current_app")
    nav_items = assigns[:nav_items] || Bonfire.Common.ExtensionModules.default_nav(current_app) || Bonfire.Common.NavModules.nav(current_app)
    |> debug("nav_items")
    ...
    |> assign_new(:without_sidebar, fn -> empty?(nav_items) && ((ulid(current_user(assigns)) && empty?(e(assigns, :sidebar_widgets, :users, :main, nil))) || (!ulid(current_user(assigns)) && empty?(e(assigns, :sidebar_widgets, :guests, :main, nil)))) end)
